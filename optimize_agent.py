...
collection.add(
    ...
    metadatas=[{"source": "AHA/ACC STEMI Guidelines"}],
    ids=["guide_01"]
)
...
# 我们的训练/测试数据集
trainset = [
    dspy.Example(clinical_note="STEMI patient. PCI done. Meds: Aspirin 81mg, Clopidogrel 75mg, Atorvastatin 40mg.",
                 expected_missing="beta-blocker").with_inputs('clinical_note'),
    dspy.Example(
        ...
        expected_missing="none").with_inputs('clinical_note'),
    dspy.Example(
        ...
        expected_missing="statin").with_inputs('clinical_note')
]
...
print("2. Starting DSPy Compiler (Optimizing the Agent)...")
# 设置优化器：最多生成2个成功的推理范例（Bootstrapped demos）插入到模型的提示词中
teleprompter = BootstrapFewShot(metric=clinical_audit_metric, max_bootstrapped_demos=2, max_labeled_demos=1)

# 开始编译！DSPy 会自动运行验证并修正模型的思维方式
compiled_agent = teleprompter.compile(UltimateClinicalAgent(), trainset=trainset)

print("\n3. Testing the Compiled Agent on the previously failed Case 1...")
failed_case = trainset[0]
print(f"Input: {failed_case.clinical_note}")

# 使用编译后的 Agent 再次预测
optimized_pred = compiled_agent(clinical_note=failed_case.clinical_note)

print("-" * 30)
print(f"Optimized Output: {optimized_pred.missing_recommendations}")
print(f"Optimized Rationale: {optimized_pred.rationale}")
print("=" * 60)